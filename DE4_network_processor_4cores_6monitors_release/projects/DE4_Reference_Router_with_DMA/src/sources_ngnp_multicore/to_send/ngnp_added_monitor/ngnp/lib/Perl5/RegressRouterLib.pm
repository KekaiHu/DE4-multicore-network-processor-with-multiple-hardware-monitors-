...
#
#
# Invoke using: use NF2::RegressRouterLib
#
# Module provides NF2-specific Router functions
#
# Revisions:
#
##############################################################
...
sub nftest_set_router_MAC {
	...
	$portNum =~ s/nf2c//;
	$portNum = ( $portNum % 4 ) + 1;
	my @reg_access = NF2::RegressLib::nftest_get_hw_reg_access($ifaceName);
...
sub nftest_add_LPM_table_entry {
	my @reg_access = NF2::RegressLib::nftest_get_hw_reg_access(shift);
...
sub nftest_contains_LPM_table_entries {
	my @reg_access       = NF2::RegressLib::nftest_get_hw_reg_access("nf2c0");
